...
class Doctor_Sync {
	...
	/**
	 * @param array<int,array<string,mixed>> $doctors
	 * @return array<string,mixed>
	 */
	public function sync( array $doctors ): array {
		$results = array(
			'processed'      => 0,
			'created'        => 0,
			'updated'        => 0,
		...
		foreach ( $doctors as $doctor ) {
			...
			$post_data = array(
				'post_title'   => sanitize_text_field( (string) ( $doctor['full_name'] ?? 'Doctor' ) ),
			...
			if ( ! empty( $doctor['photo_url'] ) ) {
				$image_id = Image_Importer::import_doctor_photo( (string) $doctor['photo_url'], $organization_id, $doctor_slug, (int) $post_id );
	...
	private function save_meta( int $post_id, array $doctor ): bool {
		...
		$meta_map = array(
			...
			'doctor_name'          => sanitize_text_field( (string) ( $doctor['full_name'] ?? '' ) ),
